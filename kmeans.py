...
class KMeans(object):
    ...
    def initialize_centroids(self, data):
        ...
        for i in range(1, self.k):
            ...
            for j in range(len(self.centroids)):
                distances[j] = self.get_euclidean_distance(sliced_data, self.centroids.iloc[j, self.start_var:self.end_var])
    ...
    def get_euclidean_distance(self, point1, point2):
        ...
        qminusp = (point1 - point2)**2
        if isinstance(point1, pd.DataFrame):
            return pd.Series(np.sqrt(qminusp.sum(axis =1)))
        else:
            return np.sqrt(qminusp.sum())
    ...
    def group_observations(self, data):
        ...
        for i in range(self.k):
            ...
            # Hint: Use the get_euclidean_distance() function that we have
            # defined in this class.
            distances[i] = self.get_euclidean_distance(sliced_data, self.centroids.iloc[i, self.start_var:self.end_var])
    ...
    def adjust_centroids(self, data, groups):
        ...
        return centroids.drop(['class'], axis=1)
